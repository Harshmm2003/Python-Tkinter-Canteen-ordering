...
def login():
	if user_name.get()=="" or password.get()=="":
		...
		try:
			...
			cur.execute("select * from userdata where user_name=%s and password = %s",(user_name.get(),password.get()))
			row = cur.fetchone()
	...
	def action():
		if first_name.get()=="" or last_name.get()=="" or age.get()=="" or sapid.get()=="" or campus.get()=="" or user_name.get()=="" or password.get()=="" or very_pass.get()=="":
			...
			try:
				con = pymysql.connect(host="localhost",user="root",password="",database="canteen")
				cur = con.cursor()
				cur.execute("select * from userdata where user_name=%s",user_name.get())
				row = cur.fetchone()
				if row!=None:
					messagebox.showerror("Error" , "User Name Already Exists", parent = winsignup)
				else:
					cur.execute("insert into userdata(first_name,last_name,user_name,password) values(%s,%s,%s,%s)",
						(
						first_name.get(),
						last_name.get(),
						user_name.get(),
